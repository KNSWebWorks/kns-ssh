...
- **tmux-режим**: если на машине агента установлен `tmux` (на macOS: `brew install tmux`), каждая сессия — это tmux-сессия. Шеллы **переживают рестарт и обновление агента**, replay берётся из истории tmux (2000 строк). Без tmux — fallback на обычный bash (сессии живут до рестарта агента).
- **Restart**: кнопка ⟳ пересоздаёт shell текущей вкладки.
- **Клавиши**: Ctrl+C (без выделения — SIGINT, с выделением — копирование), Ctrl+Z/X/A/D/L/E/U/K/W/R, Ctrl+V / Cmd+V — вставка. Панель спецклавиш (Esc/Tab/^C/^D/стрелки + залипающий Ctrl) для телефона.
...
- Токен агента — секрет агента: зная его, можно подключиться как агент (консоль при этом откроется только владельцу — клиентский WS требует логин и проверяет принадлежность агента пользователю).
- Токены агентов хранятся в базе открытым текстом (хэширование — в планах).
- Сессии в tmux-режиме живут в tmux-сервере машины агента и переживают рестарт агента; перезагрузка машины убивает их. Без tmux сессии живут в памяти агента.

## Безопасность

- `GET /api/agents/online` — только со валидным PocketBase-токеном, только свои агенты.
- `GET /api/ws/agent` — только с токеном существующего агента.
- `GET /api/ws/client` — после upgrade требуется первое сообщение `{"type":"auth","data":"<PB token>"}` (10 сек на ответ); сервер проверяет токен и принадлежность агента пользователю, иначе шлёт `auth_error` и закрывает соединение. Токен не передаётся в URL и не попадает в access-логи.
